...
with open('/home/haka/old/SentiMovie/search.csv','r') as f:
    reader=csv.reader(f)
...
class reviewspider(scrapy.Spider):
    ...
    def after_results(self,response):
        links= response.css('.result::attr(href)').extract()
        print(links)
        for link in links[:5]:
            #print(link)
    ...
    def parse_data(self,response):
         ...
         budget=response.css("p:nth-child(4)::text").extract()
         #print(budget)

         revenue=response.css("p:nth-child(5)::text").extract()
         #print(revenue)
